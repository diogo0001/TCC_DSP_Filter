...
% Options ------------
play = 0;
write_file = 2; % 1 single or 2 for cascade crossover
...
f0_c = 350;
Q_c = 0.5;   
%-3db   0.707 -> Butterrworth 2th order
%-6db   0.5   -> Butterrworth 2th order
...
% Transfer function analisis -----------------------------
    [ftma_L,ftmf_L] = analise(bl,al,fig,0,0,0,fs);
    [ftma_H,ftmf_H] = analise(bh,ah,fig,0,0,0,fs);
    options = bodeoptions;
    ...
    bode(ftma_L,options);
    hold on;
    bode(-ftma_H,options);  % Inverted the polarization 
    hold on;
    bode(ftma_H,options);
...
function [lp,hp] = cros_coef_calc(f0,G,Q,fs,type)
    ...
    switch type
        case 'butt-2nd'
            ...
            hp(1) = -((1+cos(w0))/2);   % Inverted the polarization
            hp(2) = (1+cos(w0));        % Inverted the polarization
            hp(3) = hp(1);              % Inverted the polarization
            hp(4) = a0;
            hp(5) = (-2.0*cos(w0));
            hp(6) = (1 - alpha);  
