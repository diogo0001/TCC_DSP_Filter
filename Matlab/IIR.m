...
if eq == 1
    ...
    if write_file == 1
        save_file = strcat(input_folder,output_folder,'iir_EQ',int2str(f0),file);
...
if crossover == 1
    ...
    [lp, hp] = cros_coef_calc(f0,G,Q,fs);
    stereo_mtx = cross(lp,hp,audio,coef_amp);

    if write_file == 1
        save_file = strcat(input_folder,output_folder,'iir_cross_',int2str(f0),file);
